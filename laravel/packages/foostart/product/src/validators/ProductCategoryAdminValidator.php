...
class ProductCategoryAdminValidator extends AbstractValidator
{
    ...
    public function isValidTitle($input) {
        ...
        $min_lenght = config('product_admin_.name_min_lengh');
        $max_lenght = config('product_admin_.name_max_lengh');

        $product_category_name = @$input['product_category_name'];

        if ((strlen($product_category_name) <= $min_lenght)  || ((strlen($product_category_name) >= $max_lenght))) {
